...
void CCM_wrapping3(double *A, double *Aest, double *B, double *rho, int *pE, int *ptau, int *plengtht, int *pLibLength) {
    int i, j, k, l, from, to, slide, count, slidetrip=0, nloop;
    ...
    /* *neighbors, *u, and *w are all (E+1) long */
        ...
        for(l=tau*(E-1)+(E+1); l<lengtht; l++) { /* #### try out all possible library sizes ((E+1) to LibLengh-tau*(E-1)) #### */
            to=l;
			if(slidetrip==0) {
			count=from;
